...
read JAR < $filename
xToken=$1
curl -X POST -H "X-Api-Token: $xToken" -F 'metadata={"changelog":"Auto upload by TravisCI, The snapshot may damage your server, not working, destory your saves and databases, backup and use it at your own risk.","gameVersions":[7330,7105],"releaseType":"alpha"}' -F "file=@./devbukkit_push/$JAR" -s "https://dev.bukkit.org/api/projects/320536/upload-file" > fileid.json
cat fileid.json
