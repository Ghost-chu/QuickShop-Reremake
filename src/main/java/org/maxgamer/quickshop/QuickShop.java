...
public class QuickShop extends JavaPlugin {
   ...
   */
  @Getter
  private final CompatibilityManager compatibilityTool =
      new CompatibilityManager(this);
  /**
   * The shop limites.
   */
  @Getter private final Map<String, Integer> limits = new HashMap<>(15);
  private final ConfigProvider configProvider = new ConfigProvider(this);
  boolean onLoadCalled = false;
  @Getter private IntegrationHelper integrationHelper;
  /**
   * The BootError, if it not NULL, plugin will stop loading and show setted
   * errors when use /qs
   */
  @Nullable @Getter @Setter private BootError bootError;
  @Getter private CommandManager commandManager;
  /**
   * Contains all SQL tasks
   */
  @Getter private DatabaseHelper databaseHelper;
  /**
   ...
   */
  @Getter private boolean display = true;
  @Getter private int displayItemCheckTicks;
  @Getter private DisplayWatcher displayWatcher;
  /**
   * The economy we hook into for transactions
   */
  @Getter private Economy economy;
  @Getter private ItemMatcher itemMatcher;
  /**
   ...
   */
  @Getter private boolean limit = false;
  // private BukkitTask itemWatcherTask;
  @Nullable @Getter private LogWatcher logWatcher;
  /**
   ...
   */
  @Getter private Plugin openInvPlugin;
  /**
   * The plugin PlaceHolderAPI(null if not present)
   */
  @Getter private Plugin placeHolderAPI;
  /**
   * A util to call to check some actions permission
   */
  @Getter private PermissionChecker permissionChecker;
  /**
   * Whether we players are charged a fee to change the price on their shop (To
   * help deter endless undercutting
   */
  @Getter private boolean priceChangeRequiresFee = false;
  /**
   * The error reporter to help devs report errors to Sentry.io
   */
  @Getter private RollbarErrorReporter sentryErrorReporter;
  /**
   * The server UniqueID, use to the ErrorReporter
   */
  @Getter private UUID serverUniqueID;
  private boolean setupDBonEnableding = false;
  /**
   * Rewrited shoploader, more faster.
   */
  @Getter private ShopLoader shopLoader;
  /**
   * The Shop Manager used to store shops
   */
  @Getter private ShopManager shopManager;
  @Getter private SyncTaskWatcher syncTaskWatcher;
  // private ShopVaildWatcher shopVaildWatcher;
  @Getter private DisplayAutoDespawnWatcher displayAutoDespawnWatcher;
  @Getter private OngoingFeeWatcher ongoingFeeWatcher;
  @Getter private SignUpdateWatcher signUpdateWatcher;
  @Getter private ShopContainerWatcher shopContainerWatcher;
  @Getter
  private @Deprecated DisplayDupeRemoverWatcher displayDupeRemoverWatcher;
  @Getter private BukkitAPIWrapper bukkitAPIWrapper;
  @Getter private boolean enabledAsyncDisplayDespawn;
  @Getter private String previewProtectionLore;
  @Getter private Plugin blockHubPlugin;
  @Getter private Plugin lwcPlugin;
  @Getter private Cache shopCache;
  @Getter private boolean allowStack;
  @Getter private EnvironmentChecker environmentChecker;
  @Getter @Nullable private UpdateWatcher updateWatcher;
  @Getter private BuildInfo buildInfo;
  @Getter private QuickChatType quickChatType = QuickChatType.BUNGEECHAT;
  @Getter private QuickChat quickChat = new BungeeQuickChat();
  @Getter @Nullable private String currency = null;
  @Getter private ShopControlPanel shopControlPanelManager;
   ...
   */
  public static String getFork() { return "Reremake"; }
  ...
  public int getShopLimit(@NotNull Player p) {
    ...
    for (Entry<String, Integer> entry : limits.entrySet()) {
      if (entry.getValue() > max &&
          getPermissionManager().hasPermission(p, entry.getKey())) {
        max = entry.getValue();
      }
    }
    return max;
  ...
  private void load3rdParty() {
    ...
    if (this.display) {
      ...
      if (DisplayItem.getNowUsing() == DisplayType.VIRTUALITEM) {
        getLogger().info(
            "Using Virtual item Display, loading ProtocolLib support...");
        ...
        if (protocolLibPlugin != null && protocolLibPlugin.isEnabled()) {
          ...
          getLogger().warning(
              "Failed to load ProtocolLib support, fallback to real item display");
      ...
      if (DisplayItem.getNowUsing() == DisplayType.REALITEM &&
        ...
        try {
          Clearlag clearlag =
              (Clearlag)Bukkit.getPluginManager().getPlugin("ClearLag");
          for (RegisteredListener clearLagListener :
            ...
            if (clearLagListener.getListener().getClass().equals(
                    ItemMergeListener.class)) {
              ItemSpawnEvent.getHandlerList().unregister(
                  clearLagListener.getListener());
              getLogger().warning(
                  "+++++++++++++++++++++++++++++++++++++++++++");
              getLogger().severe(
  ...
  private boolean loadEcon() {
    try {
      ...
      case UNKNOWN:
        bootError = new BootError(
            this.getLogger(),
            "Can't load the Economy provider, invaild value in config.yml.");
        return false;
      case VAULT:
        ...
        if (getConfig().getDouble("tax", 0) > 0) {
          ...
          try {
            ...
            if (!(taxAccount == null || taxAccount.isEmpty())) {
              OfflinePlayer tax =
                  Bukkit.getOfflinePlayer(Objects.requireNonNull(
                      taxAccount)); // FIXME: Should we use player UUID?
              if (!tax.hasPlayedBefore()) {
                Economy_Vault vault = (Economy_Vault)core;
                if (vault.isValid()) {
                  if (!Objects.requireNonNull(vault.getVault())
                           .hasAccount(tax)) {
                    try {
                    ...
                    if (!vault.getVault().hasAccount(tax)) {
                      getLogger().warning(
                          "Tax account's player never played this server before, that may cause server lagg or economy system error, you should change that name. But if this warning not cause any issues, you can safety ignore this.");
                    }
                  }
                }
              }
      ...
      case TNE:
        ...
        //                    Util.debugLog("Now using the Mixed economy
        //                    system.");
      default:
      ...
      getLogger().log(Level.WARNING,
                      "Something going wrong when loading up economy system",
                      e);
      getLogger().severe(
          "QuickShop could not hook into a economy/Not found Vault or Reserve!");
      getLogger().severe("QuickShop CANNOT start!");
      bootError = BuiltInSolution.econError();
      HandlerList.unregisterAll(this);
      getLogger().severe(
          "Plugin listeners was disabled, please fix the economy issue.");
      return false;
    }
  }
  ...
  public void reloadConfig() {
    ...
    this.priceChangeRequiresFee =
        this.getConfig().getBoolean("shop.price-change-requires-fee");
    this.displayItemCheckTicks =
        this.getConfig().getInt("shop.display-items-check-ticks");
    this.allowStack = this.getConfig().getBoolean("shop.allow-stacks");
    this.quickChatType =
        QuickChatType.fromID(this.getConfig().getInt("chat-type"));
  ...
  public void onLoad() {
    ...
    if (getConfig().getBoolean("debug.adventure", false)) {
      ...
      getLogger().warning(
          "Adventure debug flag was set! You can disable this anytime in config by set `debug.adventure` to false.");
    }
    this.bootError = null;
    ...
    if (getConfig().getBoolean("integration.worldguard.enable")) {
      ...
      if (wg != null) {
        ...
        this.integrationHelper.register(new WorldGuardIntegration(
            this)); // WG require register flags when onLoad called.
      }
    }
  ...
  public void onDisable() {
    ...
    getLogger().info(
        "QuickShop is finishing remaining work, this may need a while...");
    //        if (sentryErrorReporter != null &&
    //        sentryErrorReporter.isEnabled()) {
    //            Paste paste = new Paste(this);
    //            String lastPaste = paste.paste(paste.genNewPaste(), 1);
    //            if (lastPaste != null) {
    //                log("Plugin unloaded, the server paste was created for
    //                debugging, reporting errors and data-recovery: " +
    //                lastPaste);
    //            }
    //        }
    ...
    for (Player player : Bukkit.getOnlinePlayers()) {
      ...
      if (topInventory.getHolder() instanceof QuickShopPreviewInventoryHolder ||
          bottomInventory.getHolder() instanceof
              QuickShopPreviewInventoryHolder) {
    ...
    try {
      for (Shop shop :
           Objects.requireNonNull(this.getShopManager().getLoadedShops())) {
        if (shop.isLoaded()) {
          shop.onUnload();
        }
      }
  ...
  private void initConfiguration() {
    ...
    try {
      ...
      getLogger().severe(
          "Failed to save config.yml from jar, The binary file of QuickShop may damaged. Please re-download from our website.");
    }
    reloadConfig();
    /*
    ...
    target save file. If set to true, it will write out the default values, to
    the target file. However, once written, you will not be able to tell the
    difference between a default and a value from the configuration.
    ==========================================================================================================================
    getConfig().options().copyDefaults(true).header("Read the example.config.yml
    file to get commented example config file."); // Load defaults.
    saveDefaultConfig();
    reloadConfig();
    */
    ...
    this.serverUniqueID =
        UUID.fromString(Objects.requireNonNull(getConfig().getString(
            "server-uuid", String.valueOf(UUID.randomUUID()))));
  ...
  public void onEnable() {
    ...
    environmentChecker =
        new org.maxgamer.quickshop.util.envcheck.EnvironmentChecker(this);
    ResultReport resultReport = environmentChecker.run();
    if (resultReport.getFinalResult().ordinal() >
        CheckResult.WARNING.ordinal()) {
      StringBuilder builder = new StringBuilder();
      for (Entry<EnvCheckEntry, ResultContainer> result :
           resultReport.getResults().entrySet()) {
        if (result.getValue().getResult().ordinal() >
            CheckResult.WARNING.ordinal()) {
          builder
              .append(String.format("- [%s/%s] %s", result.getKey().name(),
    ...
    getLogger().info("Reading the configuration...");
    this.initConfiguration();
    getLogger().info("Developers: " +
                     Util.list2String(this.getDescription().getAuthors()));
    ...
    try {
      ...
      getLogger().warning("Cannot load the Sentry Error Reporter: " +
                          th.getMessage());
      getLogger().warning(
    ...
    this.previewProtectionLore =
        MsgUtil.getMessageOfflinePlayer("quickshop-gui-preview", null);
    if (this.previewProtectionLore == null ||
        this.previewProtectionLore.isEmpty()) {
      this.previewProtectionLore =
          ChatColor.RED + "FIXME: DON'T SET THIS TO EMPTY STRING";
    }
    ...
    ConfigurationSection limitCfg =
        this.getConfig().getConfigurationSection("limits");
    if (limitCfg != null) {
    ...
    if (getConfig().getInt("shop.finding.distance") > 100) {
      getLogger().severe(
          "Shop find distance is too high! It may cause lag! Pick a number under 100!");
    }
    ...
    /* Delay the Ecoonomy system load, give a chance to let economy system
     * regiser. */
    ...
    if (logWatcher != null) {
      ...
      getLogger().info(
          "Log actions is enabled, actions will log in the qs.log file!");
    }
    if (getConfig().getBoolean("shop.ongoing-fee.enable")) {
      ...
      ongoingFeeWatcher.runTaskTimerAsynchronously(
          this, 0, getConfig().getInt("shop.ongoing-fee.ticks"));
    }
    ...
    Util.debugLog("Now using display-type: " +
                  DisplayItem.getNowUsing().name());
  ...
  private boolean setupDatabase() {
    try {
      ...
      if (Objects.requireNonNull(dbCfg).getBoolean("mysql")) {
        ...
        dbCore = new MySQLCore(
            this, Objects.requireNonNull(host, "MySQL host can't be null"),
            Objects.requireNonNull(user, "MySQL username can't be null"),
            Objects.requireNonNull(pass, "MySQL password can't be null"),
            Objects.requireNonNull(database,
                                   "MySQL database name can't be null"),
            Objects.requireNonNull(port, "MySQL port can't be null"), useSSL);
      } else {
        // SQLite database - Doing this handles file creation
        dbCore =
            new SQLiteCore(this, new File(this.getDataFolder(), "shops.db"));
      }
      this.databaseManager =
          new DatabaseManager(this, ServiceInjector.getDatabaseCore(dbCore));
  ...
  private void submitMeritcs() {
    if (!getConfig().getBoolean("disabled-metrics")) {
      ...
      metrics.addCustomChart(
          new Metrics.SimplePie("server_version", Bukkit::getVersion));
      metrics.addCustomChart(
          new Metrics.SimplePie("bukkit_version", Bukkit::getBukkitVersion));
      metrics.addCustomChart(
          new Metrics.SimplePie("vault_version", () -> vaultVer));
      metrics.addCustomChart(new Metrics.SimplePie(
          "use_display_items",
          ()
              -> Util.boolean2Status(
                  getConfig().getBoolean("shop.display-items"))));
      metrics.addCustomChart(new Metrics.SimplePie(
          "use_locks",
          () -> Util.boolean2Status(getConfig().getBoolean("shop.lock"))));
      metrics.addCustomChart(new Metrics.SimplePie(
          "use_sneak_action",
          ()
              -> Util.boolean2Status(
                  getConfig().getBoolean("shop.interact.sneak-to-create") ||
                  getConfig().getBoolean("shop.interact.sneak-to-trade") ||
                  getConfig().getBoolean("shop.interact.sneak-to-control"))));
      metrics.addCustomChart(new Metrics.SimplePie(
          "shop_find_distance",
          () -> getConfig().getString("shop.finding.distance")));
      String finalEconomyType = economyType;
      metrics.addCustomChart(
          new Metrics.SimplePie("economy_type", () -> finalEconomyType));
      metrics.addCustomChart(
          new Metrics.SimplePie("use_display_auto_despawn",
                                ()
                                    -> String.valueOf(getConfig().getBoolean(
                                        "shop.display-auto-despawn"))));
      metrics.addCustomChart(
          new Metrics.SimplePie("use_enhance_display_protect",
                                ()
                                    -> String.valueOf(getConfig().getBoolean(
                                        "shop.enchance-display-protect"))));
      metrics.addCustomChart(
          new Metrics.SimplePie("use_enhance_shop_protect",
                                ()
                                    -> String.valueOf(getConfig().getBoolean(
                                        "shop.enchance-shop-protect"))));
      metrics.addCustomChart(
          new Metrics.SimplePie("use_ongoing_fee",
                                ()
                                    -> String.valueOf(getConfig().getBoolean(
                                        "shop.ongoing-fee.enable"))));
      metrics.addCustomChart(new Metrics.SimplePie(
          "database_type",
          () -> this.getDatabaseManager().getDatabase().getName()));
      metrics.addCustomChart(new Metrics.SimplePie(
          "display_type", () -> DisplayItem.getNowUsing().name()));
      metrics.addCustomChart(new Metrics.SimplePie(
          "itemmatcher_type", () -> this.getItemMatcher().getName()));
      metrics.addCustomChart(new Metrics.SimplePie(
          "use_stack_item", () -> String.valueOf(this.isAllowStack())));
      metrics.addCustomChart(new Metrics.SimplePie(
          "chat_adapter", () -> this.getQuickChatType().name()));
      metrics.addCustomChart(
          new Metrics.SimplePie("event_adapter", () -> eventAdapter));
      metrics.addCustomChart(new Metrics.SingleLineChart(
          "shops_created_on_all_servers",
  ...
  private void updateConfig(int selectedVersion) {
    ...
    if (selectedVersion == 10) {
      getConfig().set("shop.pay-player-from-unlimited-shop-owner",
                      null); // Removed
    ...
    if (selectedVersion == 23) {
      ...
      getConfig().set("shop.blacklist-world",
                      Lists.newArrayList("disabled_world_name"));
    ...
    if (selectedVersion == 62) { // Ahhh fuck versions
      ...
      getConfig().set("shop.word-for-trade-all-items",
    ...
    if (selectedVersion == 72) {
      if (getConfig().getBoolean("use-deciaml-format")) {
        getConfig().set("use-decimal-format",
                        getConfig().getBoolean("use-deciaml-format"));
      } else {
        getConfig().set("use-decimal-format", false);
      }
    ...
    if (selectedVersion == 74) {
      String langUtilsLanguage =
          getConfig().getString("langutils-language", "en_us");
    ...
    if (selectedVersion == 77) {
      ...
      getConfig().set("integration.towny.create",
                      new String[] {"SHOPTYPE", "MODIFY"});
      getConfig().set("integration.towny.trade", new String[] {});
      getConfig().set("integration.worldguard.enable", false);
      getConfig().set("integration.worldguard.create",
                      new String[] {"FLAG", "CHEST_ACCESS"});
      ...
      getConfig().set("integration.residence.create",
                      new String[] {"FLAG", "interact", "use"});
    ...
    if (selectedVersion == 94) {
      if (getConfig().isSet("price-restriction")) {
        getConfig().set("shop.price-restriction",
                        getConfig().getStringList("price-restriction"));
    ...
    if (selectedVersion == 105) {
      getConfig().set("shop.interact.sneak-to-create",
                      getConfig().getBoolean("shop.sneak-to-create"));
      getConfig().set("shop.sneak-to-create", null);
      getConfig().set("shop.interact.sneak-to-trade",
                      getConfig().getBoolean("shop.sneak-to-trade"));
      getConfig().set("shop.sneak-to-trade", null);
      getConfig().set("shop.interact.sneak-to-control",
                      getConfig().getBoolean("shop.sneak-to-control"));
    ...
    if (selectedVersion == 109) {
      getConfig().set("shop.protection-checking-blacklist",
                      Collections.singletonList("disabled_world"));
    ...
    if (selectedVersion == 111) {
      ...
      getConfig().set("logging.log-actions",
                      getConfig().getBoolean("log-actions"));
    ...
    if (selectedVersion == 114) {
      getConfig().set("shop.interact.interact-mode",
                      getConfig().getBoolean("shop.interact.switch-mode") ? 0
                                                                          : 1);
    ...
    if (selectedVersion == 115) {
      ...
      getConfig().set("integration.griefprevention.create",
                      Collections.emptyList());
      getConfig().set("integration.griefprevention.trade",
                      Collections.emptyList());
    ...
    if (selectedVersion == 117) {
      getConfig().set("shop.finding.distance",
                      getConfig().getInt("shop.find-distance"));
    ...
    if (selectedVersion == 122) {
      ...
      getConfig().set("alternate-currency-symbol-list",
                      Arrays.asList("CNY;¥", "USD;$"));
      getConfig().set("config-version", ++selectedVersion);
    }
    ...
    if (getConfig().getInt("matcher.work-type") != 0 &&
        GameVersion.get(ReflectFactory.getServerVersion())
            .name()
            .contains("1_16")) {
      getLogger().warning(
    ...
    InputStreamReader buildInConfigReader =
        new InputStreamReader(new BufferedInputStream(
            Objects.requireNonNull(getResource("config.yml"))));
    new ConfigurationFixer(
        this, YamlConfiguration.loadConfiguration(buildInConfigReader))
        .fix();

    saveConfig();
    reloadConfig();

    Path exampleConfigFile =
        new File(getDataFolder(), "example.config.yml").toPath();
    try {
      Files.copy(Objects.requireNonNull(getResource("config.yml")),
                 exampleConfigFile, REPLACE_EXISTING);
    } catch (IOException ioe) {
      getLogger().warning("Error when creating the example config file: " +
                          ioe.getMessage());
    }
  }
}
