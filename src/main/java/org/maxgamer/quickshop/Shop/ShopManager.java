...
public class ShopManager {
    ...
    private void actionTrade(Player p, HashMap<UUID, Info> actions, Info info, String message) {
        ...
        if (shop == null || Util.canBeShop(info.getLocation().getBlock(), null) == false) {
    ...
    private void actionBuy(Player p, HashMap<UUID, Info> actions2, Info info, String message, Shop shop, int amount) {
        ...
        if (space < amount) {
            p.sendMessage(MsgUtil
                    .getMessage("shop-has-no-space", "" + space, MsgUtil.getItemi18n(shop.getItem().getType().name())));
            return;
        }
        int count = Util.countItems(p.getInventory(), shop.getItem());
        // Not enough items
        if (amount > count) {
            p.sendMessage(MsgUtil.getMessage("you-dont-have-that-many-items", "" + count, MsgUtil
                    .getItemi18n(shop.getItem().getType().name())));
            return;
        }
        if (amount == 0) {
            // Dumb.
        ...
        if (!p.getUniqueId().equals(shop.getOwner())) {
            ...
            String msg = MsgUtil.getMessage("player-sold-to-your-store", p.getName(), "" + amount, MsgUtil
                    .getItemi18n(shop.getItem().getType().name()));
            if (space == amount)
    ...
    private void actionSell(Player p, HashMap<UUID, Info> actions2, Info info, String message, Shop shop2, int amount) {
        Shop shop = plugin.getShopManager().getShop(info.getLocation());
        // It's not valid anymore
        if (shop == null || !Util.canBeShop(info.getLocation().getBlock(), null)) {
            p.sendMessage(MsgUtil.getMessage("chest-was-removed"));
            return;
        }
        if (info.hasChanged(shop)) {
        ...
        if (stock == -1)
            stock = 10000;
        if (stock < amount) {
            p.sendMessage(MsgUtil.getMessage("shop-stock-too-low", "" + shop.getRemainingStock(), MsgUtil
                    .getItemi18n(shop.getItem().getType().name())));
            return;
        }
        if (amount == 0) {
        ...
        if (!p.getUniqueId().equals(shop.getOwner())) {
            ...
            if (plugin.getConfig().getBoolean("show-tax")) {
                String msg = MsgUtil.getMessage("player-bought-from-your-store-tax", p.getName(), "" + amount, MsgUtil
                        .getItemi18n(shop.getItem().getType().name()), Util.format((tax * total)));
                if (stock == amount)
                    msg += "\n" + MsgUtil.getMessage("shop-out-of-stock", "" + shop.getLocation().getBlockX(), "" + shop
                            .getLocation().getBlockY(), "" + shop.getLocation().getBlockZ(), MsgUtil
                            .getItemi18n(shop.getItem().getType().name()));
                MsgUtil.send(shop.getOwner(), msg, shop.isUnlimited());
            } else {
                String msg = MsgUtil.getMessage("player-bought-from-your-store", p.getName(), "" + amount, MsgUtil
                        .getItemi18n(shop.getItem().getType().name()));
                if (stock == amount)
                    msg += "\n" + MsgUtil.getMessage("shop-out-of-stock", "" + shop.getLocation().getBlockX(), "" + shop
                            .getLocation().getBlockY(), "" + shop.getLocation().getBlockZ(), MsgUtil
    ...
    private void actionCreate(Player p, HashMap<UUID, Info> actions2, Info info, String message) {
        ...
        try {
            ...
            if (Util.canBeShop(info.getLocation().getBlock(), p.getUniqueId()) == false) {
                p.sendMessage(MsgUtil.getMessage("chest-was-removed"));
                return;
            }
