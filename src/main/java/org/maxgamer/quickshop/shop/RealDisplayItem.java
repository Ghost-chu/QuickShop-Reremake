...
public class RealDisplayItem extends DisplayItem {
  ...
  public boolean checkDisplayIsMoved() {
    ...
    /* We give 0.6 block to allow item drop on the chest, not floating on the
     * air. */
  ...
  public void fixDisplayMovedOld() {
    ...
    for (Entity entity :
      ...
      if (eItem.getUniqueId().equals(
              Objects.requireNonNull(this.item).getUniqueId())) {
        Util.debugLog("Fixing moved Item displayItem " + eItem.getUniqueId() +
                      " at " + eItem.getLocation());
        plugin.getBukkitAPIWrapper().teleportEntity(
            eItem, Objects.requireNonNull(getDisplayLocation()),
  ...
  public void remove() {
    ...
    if (this.item == null) {
      Util.debugLog(
          "Ignore the Item removing because the Item is already gone.");
      return;
    }
    this.item.remove();
    this.item = null;
  ...
  public boolean removeDupe() {
    ...
    for (Entity entity : item.getNearbyEntities(1.5, 1.5, 1.5)) {
      ...
      if (!eItem.getUniqueId().equals(displayUUID)) {
        if (DisplayItem.checkIsTargetShopDisplay(eItem.getItemStack(),
                                                 this.shop)) {
          Util.debugLog("Removing a duped ItemEntity " + eItem.getUniqueId() +
                        " at " + eItem.getLocation());
          entity.remove();
          removed = true;
        }
  ...
  public void spawn() {
    ...
    if (!Util.isDisplayAllowBlock(Objects.requireNonNull(getDisplayLocation())
                                      .getBlock()
                                      .getType())) {
      Util.debugLog(
    ...
    this.item = this.shop.getLocation().getWorld().dropItem(
        getDisplayLocation(), this.guardedIstack);
