...
public class ShopLoader {
  ...
  private final List<ShopDatabaseInfoOrigin> originShopsInDatabase =
  ...
  public void loadShops() { loadShops(null); }
  ...
  public void loadShops(@Nullable String worldName) {
    ...
    try (WarpedResultSet warpRS = plugin.getDatabaseHelper().selectAllShops()) {
      ...
      this.plugin.getLogger().info("Used " + fetchTimer.endTimer() +
                                   "ms to fetch all shops from the database.");
      while (rs.next()) {
        ...
        Shop shop = new ContainerShop(plugin, data.getLocation(),
                                      data.getPrice(), data.getItem(),
                                      data.getModerators(), data.isUnlimited(),
                                      data.getType(), data.getExtra());
        ...
        if (Util.isLoaded(shop.getLocation())) {
          ...
          if (!Util.canBeShop(shop.getLocation().getBlock())) {
            Util.debugLog(
                "Target block can't be a shop, removing it from the memory...");
            // shop.delete();
            plugin.getShopManager().removeShop(shop); // Remove from Mem
            // TODO: Only remove from memory, so if it actually is a bug, user
            // won't lost all shops.
            // TODO: Old shop will be deleted when in same location creating new
            // shop.
            //                        if (!backupedDatabaseInDeleteProcess) { //
            //                        Only backup db one time.
            //                            backupedDatabaseInDeleteProcess =
            //                            Util.backupDatabase(); if
            //                            (backupedDatabaseInDeleteProcess) {
            //                                plugin.log("[SHOP LOADER] Removing
            //                                shop in the database: " +
            //                                shop.toString() + " - The block
            //                                can't be shop");
      ...
      this.plugin.getLogger().info("Successfully loaded " + totalLoaded +
                                   " shops! (Used " + totalUsedTime +
                                   "ms, Avg " + avgPerShop + "ms per shop)");
  ...
  private void singleShopLoaded(@NotNull Timer singleShopLoadTimer) {
    ...
    //            warningSender.sendWarn("Database performance bottleneck:
    //            Detected slow database, it may mean bad network connection,
    //            slow database server or database fault. Please check the
    //            database!");
    //        }
  }
  ...
  private boolean shopNullCheck(@Nullable Shop shop) {
    ...
    if (Bukkit.getOfflinePlayer(shop.getOwner()).getName() == null) {
      Util.debugLog(
          "Shop owner not exist on this server, did you have reset the playerdata?");
    }
    return false;
  }
  ...
  private void exceptionHandler(@NotNull Exception ex,
                                @Nullable Location shopLocation) {
    errors++;
    ...
    logger.warning("Location: " +
                   ((shopLocation == null) ? "NULL" : shopLocation.toString()));
  ...
  public synchronized void recoverFromFile(@NotNull String fileContent) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      try {
        ShopDatabaseInfoOrigin shopDatabaseInfoOrigin =
            gson.fromJson(shopStr, ShopDatabaseInfoOrigin.class);
      ...
      plugin.getLogger().info("Processed " + i + "/" + total + " - [" +
                              success + "]");
    }
  }
  ...
  public class ShopDatabaseInfo {
    ...
    private @Nullable
    ShopModerator deserializeModerator(@NotNull String moderatorJson) {
      ShopModerator shopModerator;
      if (Util.isUUID(moderatorJson)) {
        Util.debugLog("Updating old shop data... for " + moderatorJson);
        shopModerator =
            new ShopModerator(UUID.fromString(moderatorJson)); // New one
      } else {
        try {
          ...
          moderatorJson =
              Bukkit.getOfflinePlayer(moderatorJson).getUniqueId().toString();
          shopModerator =
              new ShopModerator(UUID.fromString(moderatorJson)); // New one
        }
      }
  ...
  public class ShopDatabaseInfoOrigin {
    ...
    ShopDatabaseInfoOrigin(int x, int y, int z, String world, String itemConfig,
                           String owner, double price, int type,
                           boolean unlimited, String extra) {
      this.x = x;
      this.y = y;
      this.z = z;
      this.world = world;
