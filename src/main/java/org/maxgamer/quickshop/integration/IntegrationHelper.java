...
public class IntegrationHelper extends QuickShopInstanceHolder {
  @Getter
  private static final Map<String, Class<? extends IntegratedPlugin>>
      integratedPluginNameMap = new HashMap<>(7);

  static {
    integratedPluginNameMap.put("Factions", FactionsUUIDIntegration.class);
    integratedPluginNameMap.put("GriefPrevention",
                                GriefPreventionIntegration.class);
    integratedPluginNameMap.put("Lands", LandsIntegration.class);
    integratedPluginNameMap.put("PlotSquared",
                                PlotSquaredIntegrationProxy.class);
  ...
  public IntegrationHelper(QuickShop plugin) { super(plugin); }
  ...
  public void searchAndRegisterPlugins() {
    ...
    for (Map.Entry<String, Class<? extends IntegratedPlugin>> entry :
         integratedPluginNameMap.entrySet()) {
      String pluginName = entry.getKey();
      if (pluginManager.isPluginEnabled(pluginName) &&
          plugin.getConfig().getBoolean("integration." +
                                        pluginName.toLowerCase() + ".enable")) {
  ...
  public void register(@NotNull IntegratedPlugin integratedPlugin) {
    ...
    if (!integrations.containsKey(integratedPlugin.getName())) {
      plugin.getLogger().info("Registering " + integratedPlugin.getName() +
                              " integration");
  ...
  public void register(
      @NotNull Class<? extends IntegratedPlugin> integratedPluginClass) {
    IntegratedPlugin integratedPlugin;
    try {
      integratedPlugin = integratedPluginClass.getConstructor(plugin.getClass())
                             .newInstance(plugin);
  ...
  public void register(@NotNull String integratedPluginName) {
    Class<? extends IntegratedPlugin> integratedPluginClass =
        integratedPluginNameMap.get(integratedPluginName);
    if (integratedPluginClass != null) {
      register(integratedPluginClass);
    } else {
      throw new InvalidIntegratedPluginClassException(
          "Invalid Integration module name: " + integratedPluginName);
    }
  }
  ...
  public void unregisterAll() {
    for (IntegratedPlugin integratedPlugin :
         new ArrayList<>(integrations.values())) {
      unregister(integratedPlugin);
    }
  }
  ...
  public void unregister(@NotNull IntegratedPlugin integratedPlugin) {
    ...
    plugin.getLogger().info("Unregistering " + integratedPlugin.getName() +
                            " integration");
  ...
  public Result callIntegrationsCanCreate(@NotNull Player player,
  ...
  public Result callIntegrationsCanTrade(@NotNull Player player,
                                         @NotNull Location location) {
    for (IntegratedPlugin plugin : integrations.values()) {
...
class InvalidIntegratedPluginClassException extends IllegalArgumentException {
  public InvalidIntegratedPluginClassException() { super(); }

  public InvalidIntegratedPluginClassException(String s) { super(s); }

  public InvalidIntegratedPluginClassException(String message,
                                               Throwable cause) {
    super(message, cause);
  }
}
