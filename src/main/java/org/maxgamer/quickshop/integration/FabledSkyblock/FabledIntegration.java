...
public class FabledIntegration extends QSIntegratedPlugin {
  ...
  public FabledIntegration(QuickShop plugin) {
    ...
    ignoreDisabledWorlds = plugin.getConfig().getBoolean(
        "integration.fabledskyblock.ignore-disabled-worlds");
    whitelist = plugin.getConfig().getBoolean(
        "integration.fabledskyblock.whitelist-mode");
  }
  ...
  public boolean canCreateShopHere(@NotNull Player player,
    ...
    Island island =
        SkyBlock.getInstance().getIslandManager().getIslandAtLocation(location);
    if (island == null)
      return whitelist;
    return island.getRole(player).equals(IslandRole.Member) ||
        island.getRole(player).equals(IslandRole.Owner) ||
  ...
  public boolean canTradeShopHere(@NotNull Player player,
                                  @NotNull Location location) {
    if (SkyBlock.getInstance().getIslandManager().getIslandAtLocation(
            location) == null) {
   ...
   */
  @Override
  public boolean canDeleteShopHere(@NotNull Player player,
                                   @NotNull Location location) {
    Island island =
        SkyBlock.getInstance().getIslandManager().getIslandAtLocation(location);
    if (island == null)
