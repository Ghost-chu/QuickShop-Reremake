...
public class InternalListener implements Listener {
  ...
  public boolean isForbidden(Material shopMaterial, Material itemMaterial) {
    ...
    return shopMaterial.isBlock() &&
        shopMaterial.name().toUpperCase().endsWith("SHULKER_BOX");
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void shopCreate(ShopCreateEvent event) {
    if (isForbidden(event.getShop().getLocation().getBlock().getType(),
                    event.getShop().getItem().getType())) {
      event.setCancelled(true);
      MsgUtil.sendMessage(
          event.getCreator(),
          MsgUtil.getMessage("forbidden-vanilla-behavior", null));
      return;
    }
    if (loggingAction) {
  ...
  public void shopModeratorChanges(ShopModeratorChangedEvent event) {
    if (loggingAction) {
      plugin.log("Shop at location " + event.getShop().getLocation() +
                 " moderator was changed to " + event.getModerator());
    }
  }
  ...
  public void shopPriceChanges(ShopPriceChangeEvent event) {
    if (loggingAction) {
      plugin.log("Shop at location " + event.getShop().getLocation() +
                 " price was changed from " + event.getOldPrice() + " to " +
                 event.getNewPrice());
    }
  }
  ...
  public void shopPrePurchase(ShopPurchaseEvent event) {
    ...
    if (loggingBalance) {
      ...
      plugin.log("Player " +
                 ...
                 " had " +
                 plugin.getEconomy().getBalance(
                     event.getPurchaser(),
                     event.getShop().getLocation().getWorld(),
                     event.getShop().getCurrency()) +
                 " before trading.");
      plugin.log("Shop Owner " + event.getShop().ownerName() + " had " +
                 plugin.getEconomy().getBalance(
    ...
    if (loggingAction) {
      if (event.getShop().getShopType() == ShopType.BUYING) {
        plugin.log(
            "Player " +
            (creator != null ? creator.getName() : event.getPurchaser()) +
            " sold " + event.getShop().ownerName() + " shop " +
            event.getShop() + " for" +
            Util.getItemStackName(event.getShop().getItem()) + "x" +
            +event.getAmount() + " for " + event.getBalance() + " (" +
            event.getTax() + " tax).");
      }
      if (event.getShop().getShopType() == ShopType.SELLING) {
        plugin.log(
            "Player " +
            (creator != null ? creator.getName() : event.getPurchaser()) +
            " bought " + event.getShop().ownerName() + " shop " +
            event.getShop() + " for " +
            Util.getItemStackName(event.getShop().getItem()) + " x" +
            event.getAmount() + " for " + event.getBalance() + " (" +
            event.getTax() + " tax).");
