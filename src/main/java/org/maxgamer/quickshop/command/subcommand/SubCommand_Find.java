  ...
  public void onCommand(@NotNull CommandSender sender,
    ...
    if (cmdArg.length == 0) {
      MsgUtil.sendMessage(sender,
                          MsgUtil.getMessage("command.no-type-given", sender));
      return;
    }
    ...
    final double maxDistance =
        plugin.getConfig().getInt("shop.finding.distance");
    final boolean usingOldLogic =
        plugin.getConfig().getBoolean("shop.finding.oldLogic");
    final int shopLimit =
        usingOldLogic ? 1 : plugin.getConfig().getInt("shop.finding.limit");
    final boolean allShops = plugin.getConfig().getBoolean("shop.finding.all");
    final boolean excludeOutOfStock =
        plugin.getConfig().getBoolean("shop.finding.exclude-out-of-stock");
    ...
    for (Shop shop : scanPool) {
      ...
      if (distance <= maxDistance) {
        ...
        if (!Util.getItemStackName(shop.getItem())
                 .toLowerCase()
                 .contains(lookFor)) {
          if (!shop.getItem().getType().name().toLowerCase().contains(
                  lookFor)) {
            continue;
          }
        }
    ...
    if (aroundShops.isEmpty()) {
      MsgUtil.sendMessage(
          sender, MsgUtil.getMessage("no-nearby-shop", sender, lookFor));
      return;
    }

    // Okay now all shops is our wanted shop in Map

    List<Map.Entry<Shop, Double>> sortedShops =
        aroundShops.entrySet()
            .stream()
    ...
    if (usingOldLogic) {
      ...
      Location lookAt =
          closest.getKey().getLocation().clone().add(0.5, 0.5, 0.5);
      plugin.getBukkitAPIWrapper().teleportEntity(
          p, Util.lookAt(p.getEyeLocation(), lookAt).add(0, -1.62, 0),
          PlayerTeleportEvent.TeleportCause.UNKNOWN);
      MsgUtil.sendMessage(p, MsgUtil.getMessage("nearby-shop-this-way", sender,
                                                closest.getValue().intValue()));
    } else {
      StringBuilder stringBuilder =
          new StringBuilder(
              MsgUtil.getMessage("nearby-shop-header", sender, lookFor))
              .append("\n");
      for (Map.Entry<Shop, Double> shopDoubleEntry : sortedShops) {
        ...
        stringBuilder
            .append(MsgUtil.getMessage(
                "nearby-shop-entry", sender, shop.getSignText()[1],
                shop.getSignText()[3], location.getBlockX(),
