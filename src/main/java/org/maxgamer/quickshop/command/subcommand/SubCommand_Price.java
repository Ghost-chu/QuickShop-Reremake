...
public class SubCommand_Price implements CommandProcesser {
  ...
  public void onCommand(@NotNull CommandSender sender,
    ...
    try { // TODO Migrate to PriceLimiter
      if (plugin.getConfig().getBoolean("whole-number-prices-only")) {
        try {
          ...
          MsgUtil.sendMessage(
              p, MsgUtil.getMessage("not-a-integer", p, cmdArg[0]));
          return;
        }
      } else {
    ...
    if (plugin.getConfig().getBoolean("shop.allow-free-shop")) {
      if (price != 0 && price < minPrice) {
        MsgUtil.sendMessage(
            p, MsgUtil.getMessage("price-too-cheap", p,
                                  (format) ? MsgUtil.decimalFormat(minPrice)
    ...
    final double price_limit =
        plugin.getConfig().getDouble("shop.maximum-price");

    if (price_limit != -1 && price > price_limit) {
      MsgUtil.sendMessage(
          p, MsgUtil.getMessage("price-too-high", p,
                                (format) ? MsgUtil.decimalFormat(price_limit)
                                         : Double.toString(price_limit)));
      return;
    }
    ...
    while (bIt.hasNext()) {
      ...
      if (shop.getPrice() == price) {
        ...
        MsgUtil.sendMessage(sender,
                            MsgUtil.getMessage("no-price-change", sender));
        return;
      }

      if (fee > 0 && !plugin.getEconomy().withdraw(
                         p.getUniqueId(), fee, shop.getLocation().getWorld(),
                         shop.getCurrency())) {
        MsgUtil.sendMessage(
            sender,
            MsgUtil.getMessage(
                "you-cant-afford-to-change-price", sender,
                plugin.getEconomy().format(fee, shop.getLocation().getWorld(),
                                           shop.getCurrency())));
        return;
      }
      ...
      if (fee > 0) {
        MsgUtil.sendMessage(
            sender, MsgUtil.getMessage("fee-charged-for-price-change", sender,
                                       plugin.getEconomy().format(
                                           fee, shop.getLocation().getWorld(),
                                           shop.getCurrency())));
        try {
          // noinspection deprecation
          plugin.getEconomy().deposit(
              plugin.getServer()
                  .getOfflinePlayer(Objects.requireNonNull(
                      plugin.getConfig().getString("tax-account")))
                  .getUniqueId(),
      ...
      MsgUtil.sendMessage(
          sender,
          MsgUtil.getMessage("price-is-now", sender,
                             plugin.getEconomy().format(
      ...
      if (cs.isSelling()) {
        if (cs.getPrice() < nextTo.getPrice()) {
          MsgUtil.sendMessage(
              sender, MsgUtil.getMessage("buying-more-than-selling", sender));
        }
      }
      // Buying
  ...
  public List<String> onTabComplete(@NotNull CommandSender sender,
                                    @NotNull String commandLabel,
                                    @NotNull String[] cmdArg) {
    return cmdArg.length == 1 ? Collections.singletonList(MsgUtil.getMessage(
                                    "tabcomplete.price", sender))
