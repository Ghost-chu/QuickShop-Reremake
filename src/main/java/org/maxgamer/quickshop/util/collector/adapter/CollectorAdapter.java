...
public class CollectorAdapter {
  ...
  public Map<?, ?> collectQuickShop(@NotNull QuickShop plugin) {
    ...
    data.put("openinv_hook",
             plugin.getOpenInvPlugin() == null ? "Disabled" : "Enabled");
  ...
  public Map<?, ?> collectModules(@NotNull QuickShop plugin) {
    Map<Object, Object> data = new HashMap<>();
    data.put("item_matcher", plugin.getItemMatcher().getName() + "@" +
                                 plugin.getItemMatcher().getPlugin().getName());

    if (plugin.getEconomy() == null) {
      data.put("economy_core", "Not loaded@Unknown");
    } else {
      data.put("economy_core",
               plugin.getEconomy().getCore().getName() + "@" +
                   plugin.getEconomy().getCore().getPlugin().getName());
    }
    data.put(
        "database_core",
        plugin.getDatabaseManager().getDatabase().getName() + "@" +
            plugin.getDatabaseManager().getDatabase().getPlugin().getName());
    data.put("gamelanguage_processor",
             MsgUtil.gameLanguage.getName() + "@" +
                 MsgUtil.gameLanguage.getPlugin().getName());
    return data;
  }
  ...
  public Map<?, ?> collectShopsInWorld(@NotNull QuickShop plugin) {
    ...
    Bukkit.getWorlds().forEach(world -> {
      ...
      perWorld.put("items", world.getEntities()
                                .stream()
      ...
      perWorld.put("shops_in_force_loaded_chunks",
                   plugin.getShopManager()
                       .getShopsInWorld(world)
                       .stream()
                       .filter(shop
                               -> world.getForceLoadedChunks().contains(
  ...
  public Map<?, ?> collectPlugins(@NotNull QuickShop plugin) {
    ...
    Arrays.stream(Bukkit.getPluginManager().getPlugins()).forEach(pl -> {
      ...
      perPlugin.put("authors",
                    Util.list2String(pl.getDescription().getAuthors()));
      perPlugin.put("contributors",
                    Util.list2String(pl.getDescription().getContributors()));
      perPlugin.put("depend",
                    Util.list2String(pl.getDescription().getDepend()));
      perPlugin.put("soft_depend",
                    Util.list2String(pl.getDescription().getSoftDepend()));
      perPlugin.put(
          "is_addon",
          pl.getDescription().getDepend().contains("QuickShop") ||
  ...
  public Map<?, ?> collectShops(@NotNull QuickShop plugin) {
    ...
    map.put("database", plugin.getShopLoader()
                            .getOriginShopsInDatabase()
                            .stream()
                            .map(ShopLoader.ShopDatabaseInfoOrigin::toString)
                            .toArray());
    map.put("memory", plugin.getShopManager()
                          .getAllShops()
                          .stream()
                          .map(Shop::toString)
                          .toArray());
