...
public class Util {
  private static final EnumSet<Material> blacklist =
      EnumSet.noneOf(Material.class);
  private static final EnumMap<Material, Entry<Double, Double>>
      restrictedPrices = new EnumMap<>(Material.class);
  private static final EnumMap<Material, Integer> customStackSize =
      new EnumMap<>(Material.class);
  private static final EnumSet<Material> shoppables =
      EnumSet.noneOf(Material.class);
  private static final List<BlockFace> verticalFacing =
  ...
  @Getter private static boolean disableDebugLogger = false;
  ...
  public static boolean backupDatabase() {
    ...
    try {
      ...
      plugin.getLogger().log(Level.WARNING, "Failed to backup the database",
                             e1);
      return false;
    }
    return true;
   ...
   */
  public static int countItems(@Nullable Inventory inv,
   ...
   *     match for 'stackability'
   *             to occur.
   * @return The number of items that can be given to the inventory safely.
   */
  public static int countSpace(@Nullable Inventory inv,
                               @NotNull ItemStack item) {
    if (inv == null) {
      return 0;
    }
    ...
    for (final ItemStack iStack : contents) {
      if (iStack == null || iStack.getType() == Material.AIR) {
        ...
        space += iStack.getAmount() >= itemMaxStackSize
                     ? 0
                     : itemMaxStackSize - iStack.getAmount();
      }
    }
  ...
  public static int getItemMaxStackSize(@NotNull Material material) {
    return customStackSize.getOrDefault(
        material, bypassedCustomStackSize == -1 ? material.getMaxStackSize()
                                                : bypassedCustomStackSize);
  }
   ...
   */
  @Nullable
  public static ItemStack deserialize(@NotNull String config)
    ...
    try {
      ...
      if (itemDataVersion > Bukkit.getUnsafe().getDataVersion()) {
        ...
        if (plugin.getConfig().getBoolean(
          ...
          Util.debugLog("Updated, we will try load as hacked ItemStack: " +
                        config);
        } else {
          plugin.getLogger().warning(
              "Cannot load ItemStack " + config +
      ...
      throw new InvalidConfigurationException("Exception in deserialize item",
                                              e);
    }
  }
  ...
  public static void debugLog(@NotNull String... logs) {
    ...
    for (String log : logs) {
      debugLogs.add("[DEBUG] [" + className + "] [" + methodName + "] (" +
                    codeLine + ") " + log);
      QuickShop.getInstance().getLogger().info("[DEBUG] [" + className + "] [" +
                                               methodName + "] (" + codeLine +
                                               ") " + log);
    }
    lock.writeLock().unlock();
  ...
  /**
   * Formats the given number according to how vault would like it. E.g. $50 or
   * 5 dollars.
   *
   * @param n    price
   * @param shop shop
  ...
  public static String format(double n, boolean internalFormat,
                              @NotNull World world, @Nullable Shop shop) {
    if (shop != null) {
  ...
  public static String format(double n, boolean internalFormat,
    ...
    if (plugin.getEconomy() == null) {
      Util.debugLog(
          "Called format before Economy booted up, using built-in formatter.");
      return getInternalFormat(n, currency);
    }
    try {
  ...
  private static String getInternalFormat(double amount,
    ...
    if (StringUtils.isEmpty(currency)) {
      ...
      String formatted = useDecimalFormat ? MsgUtil.decimalFormat(amount)
                                          : Double.toString(amount);
      return currencySymbolOnRight ? formatted + alternateCurrencySymbol
                                   : alternateCurrencySymbol + formatted;
    } else {
  ...
  public static boolean useEnchantmentForEnchantedBook() {
    return plugin.getConfig().getBoolean(
        "shop.use-enchantment-for-enchanted-book");
  }

  @NotNull
  public static String getItemStackName(@NotNull ItemStack itemStack) {
    if (useEnchantmentForEnchantedBook() &&
  ...
  getFirstEnchantmentName(@NotNull EnchantmentStorageMeta meta) {
  ...
  public static String getPotiondata(@NotNull ItemStack iStack) {
    ...
    if (!(pMeta.getBasePotionData().isUpgraded())) {
      pEffects.add(ChatColor.BLUE +
                   MsgUtil.getPotioni18n(Objects.requireNonNull(
                       pMeta.getBasePotionData().getType().getEffectType())));
    } else {
      pEffects.add(ChatColor.BLUE +
                   MsgUtil.getPotioni18n(Objects.requireNonNull(
                       pMeta.getBasePotionData().getType().getEffectType())) +
   ...
   */
  @Nullable
  public static Entry<Double, Double>
  getPriceRestriction(@NotNull Material material) {
    return restrictedPrices.get(material);
  }

  //    /**
  //     * Returns the chest attached to the given chest. The given block must
  //     be a chest.
  //     *
  ...
  //        // if (b.getType() != Material.CHEST && b.getType() !=
  ...
  public static int getShopsInWorld(@NotNull String worldName) {
    ...
    while (iterator.hasNext()) {
      ...
      if (Objects.requireNonNull(shop.getLocation().getWorld())
              .getName()
              .equals(worldName)) {
        cost++;
      }
  ...
  public static void initialize() {
    ...
    for (String s : configBlacklist) {
      ...
      if (mat == null) {
        plugin.getLogger().warning(
            s + " is not a valid material.  Check your spelling or ID");
        continue;
      }
      blacklist.add(mat);
    }
    ...
    for (String s :
         plugin.getConfig().getStringList("shop.price-restriction")) {
      String[] sp = s.split(";");
      if (sp.length == 3) {
        try {
          ...
          restrictedPrices.put(mat, new SimpleEntry<>(Double.valueOf(sp[1]),
                                                      Double.valueOf(sp[2])));
        } catch (Exception e) {
    ...
    for (String material :
         plugin.getConfig().getStringList("custom-item-stacksize")) {
      ...
      if (mat == null || mat == Material.AIR) {
        plugin.getLogger().warning(
            material +
            " not a valid material type in custom-item-stacksize section.");
        continue;
      }
    ...
    disableDebugLogger =
        plugin.getConfig().getBoolean("debug.disable-debuglogger", false);

    currencySymbolOnRight =
        plugin.getConfig().getBoolean("shop.currency-symbol-on-right", false);
    alternateCurrencySymbol =
        plugin.getConfig().getString("shop.alternate-currency-symbol", "$");
    disableVaultFormat =
        plugin.getConfig().getBoolean("shop.disable-vault-format", false);
    useDecimalFormat =
        plugin.getConfig().getBoolean("use-decimal-format", false);

    List<String> symbols =
        plugin.getConfig().getStringList("alternate-currency-symbol-list");

    symbols.forEach(entry -> {
      String[] splits = entry.split(";", 2);
      if (splits.length < 2) {
        plugin.getLogger().warning(
            "Invalid entry in alternate-currency-symbol-list: " + entry);
      }
  ...
  private static byte[] toByteArray(@NotNull InputStream in)
      throws IOException {
        ...
        try {
          for (int i = 0; i < inv.getSize(); i++) {
            ...
            if (DisplayItem.checkIsGuardItemStack(itemStack)) {
              ...
              plugin.getSyncTaskWatcher().getInventoryEditQueue().offer(
                  new InventoryEditContainer(inv, i,
                                             new ItemStack(Material.AIR)));
              Util.debugLog(
                  "Found a displayitem in an inventory, Scheduling to removal...");
  ...
  public static boolean isBlacklisted(@NotNull ItemStack stack) {
    ...
    for (String lore : Objects.requireNonNull(stack.getItemMeta().getLore())) {
      List<String> blacklistLores =
          plugin.getConfig().getStringList("shop.blacklist-lores");
   ...
   */
  public static boolean isOtherShopWithinHopperReach(@NotNull Block b,
                                                     @NotNull Player p) {
    ...
    Shop shop =
        plugin.getShopManager().getShopIncludeAttached(bshop.getLocation());
    if (shop == null) {
      shop = plugin.getShopManager().getShopIncludeAttached(
          bshop.getLocation().clone().add(0, 1, 0));
    }
   ...
   */
  private static boolean equalsBlockStateLocation(@NotNull Location b1,
                                                  @NotNull Location b2) {
   ...
   */
  public static @NotNull Location lookAt(@NotNull Location loc,
                                         @NotNull Location lookat) {
   ...
   */
  @Deprecated
  public static boolean mapDuoMatches(@NotNull Map<?, ?> map1,
                                      @NotNull Map<?, ?> map2) {
   ...
   */
  @Deprecated
  public static boolean mapMatches(@NotNull Map<?, ?> map1,
                                   @NotNull Map<?, ?> map2) {
   ...
   */
  @Deprecated
  public static boolean listMatches(@NotNull List<?> list1,
                                    @NotNull List<?> list2) {
  ...
  /**
   * Send warning message when some plugin calling deprecated method... With the
   * trace.
   */
  @Deprecated
  ...
  public static Material getSignMaterial() {
    ...
    Material signMaterial = Material.matchMaterial(Objects.requireNonNull(
        plugin.getConfig().getString("shop.sign-material")));
    if (signMaterial != null) {
      return signMaterial;
    }
    signMaterial = Material.matchMaterial(
        "OAK_WALL_SIGN"); // Fallback default sign in 1.14
    if (signMaterial != null) {
      return signMaterial;
    }
    signMaterial =
        Material.matchMaterial("WALL_SIGN"); // Fallback default sign in 1.13
    if (signMaterial != null) {
      return signMaterial;
    }
    // What the fuck!?
    ...
    if (serverInstance == null || tpsField == null) {
      try {
        ...
        plugin.getLogger().log(
            Level.WARNING,
            "Failed to getting server TPS, please report to QuickShop.", e);
        serverInstance = null;
        tpsField = null;
    ...
    if (async) {
      if (isMainThread)
        throw new IllegalStateException(
            "#[Illegal Access] This method require runs on async thread.");
    } else {
      if (!isMainThread)
        throw new IllegalStateException(
