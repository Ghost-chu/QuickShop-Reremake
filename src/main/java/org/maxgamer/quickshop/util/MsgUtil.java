...
public class MsgUtil {
  ...
  private static final Map<UUID, LinkedList<String>> outGoingPlayerMessages =
  ...
  @Getter private static YamlConfiguration enchi18n;
  private static boolean inited;
  @Getter private static YamlConfiguration itemi18n;
  private static IFile messagei18n;
  @Getter private static YamlConfiguration potioni18n;
  private static IFile builtInLang;

  private static DecimalFormat processFormat() {
    try {
      return new DecimalFormat(Objects.requireNonNull(
          QuickShop.getInstance().getConfig().getString("decimal-format")));
    } catch (Exception e) {
      QuickShop.getInstance().getLogger().log(
          Level.WARNING,
          "Error when processing decimal format, using system default: " +
              e.getMessage());
  ...
  public static void clean() {
    plugin.getLogger().info(
        "Cleaning purchase messages from the database that are over a week old...");
    // 604800,000 msec = 1 week.
    plugin.getDatabaseHelper().cleanMessage(System.currentTimeMillis() -
                                            604800000);
  }
  ...
  public static boolean flush(@NotNull OfflinePlayer p) {
    ...
    if (player != null) {
      ...
      if (msgs != null) {
        for (String msg : msgs) {
          if (p.getPlayer() != null) {
            Util.debugLog("Accepted the msg for player " + p.getName() + " : " +
                          msg);
            ...
            if (msgData.length == 3) {
              try {
                ...
                if (data == null) {
                  ...
                  plugin.getQuickChat().sendItemHologramChat(player, msgData[0],
                                                             data, msgData[2]);
                }
   ...
   * @return message
   */
  public static String getMessageOfflinePlayer(@NotNull String loc,
    ...
    try {
      ...
      if (player != null) {
        if (plugin.getConfig().getBoolean("plugin.PlaceHolderAPI") &&
            plugin.getPlaceHolderAPI() != null &&
            plugin.getPlaceHolderAPI().isEnabled()) {
      ...
      return "Cannot load language key: " + loc +
          " because something not right, check the console for details.";
    }
  }
   ...
   * @return filled text
   */
  public static String fillArgs(@Nullable String raw,
  ...
  public static void loadGameLanguage(@NotNull String languageCode) {
    gameLanguage = ServiceInjector.getGameLanguage(
        new MojangGameLanguageImpl(plugin, languageCode));
  }
  ...
  public static void loadCfgMessages() throws InvalidConfigurationException {
    ...
    // Use try block to hook any possible exception, make sure not effect our
    // cfgMessnages code.
    ...
    if (plugin.getResource("lang/" + languageCode + "/messages.json") == null) {
      nJson = new JSONFile(plugin,
                           new File(plugin.getDataFolder(), "messages.json"),
                           "lang-original/messages.json", true);
    } else {
      if (!new File(plugin.getDataFolder(), "messages.json").exists()) {
        Util.debugLog("Creating the built-in language file to drive....");
        plugin.getLanguage().saveFile(languageCode, "messages",
                                      "messages.json");
      }
      Util.debugLog(
          "Loading up the language file from plugin i18n resources and local drive.");

      nJson = new JSONFile(plugin,
                           new File(plugin.getDataFolder(), "messages.json"),
                           "lang/" + languageCode + "/messages.json", true);
    }
    nJson.create();
    ...
    if (oldMsgFile.exists()) { // Old messages file convert.
      plugin.getLogger().info(
          "Converting the old format message.yml to message.json...");
      plugin.getLanguage().saveFile(languageCode, "messages", "messages.json");
      YamlConfiguration oldMsgI18n =
          YamlConfiguration.loadConfiguration(oldMsgFile);
      ...
      try {
        Files.move(
            oldMsgFile.toPath(),
            new File(plugin.getDataFolder(), "messages.yml.bak").toPath());
      ...
      if (!new File(plugin.getDataFolder(), "messages.json").exists()) {
        ...
        nJson.loadFromString(
            Util.readToString(new File(plugin.getDataFolder(), "messages.json")
                                  .getAbsolutePath()));
  ...
  public static void loadEnchi18n() {
    ...
    YamlConfiguration enchi18nYAML =
        YamlConfiguration.loadConfiguration(new InputStreamReader(
            Objects.requireNonNull(plugin.getResource("enchi18n.yml"))));
    ...
    for (Enchantment ench : enchsi18n) {
      String enchi18nString =
          enchi18n.getString("enchi18n." + ench.getKey().getKey().trim());
      ...
      plugin.getLogger().info("Found new ench [" + enchName +
  ...
  public static void loadItemi18n() {
    ...
    YamlConfiguration itemi18nYAML =
        YamlConfiguration.loadConfiguration(new InputStreamReader(
            Objects.requireNonNull(plugin.getResource("itemi18n.yml"))));
    ...
    for (Material material : itemsi18n) {
      ...
      plugin.getLogger().info("Found new items/blocks [" + itemName +
                              "] , adding it to the config...");
    }
    try {
  ...
  public static void loadPotioni18n() {
    ...
    YamlConfiguration potioni18nYAML =
        YamlConfiguration.loadConfiguration(new InputStreamReader(
            Objects.requireNonNull(plugin.getResource("potioni18n.yml"))));
    ...
    for (PotionEffectType potion : PotionEffectType.values()) {
      ...
      String potionI18n =
          potioni18n.getString("potioni18n." + potion.getName());
      if (potionI18n != null && !potionI18n.isEmpty()) {
        continue;
      }
      String potionName = gameLanguage.getPotion(potion);
      plugin.getLogger().info("Found new potion [" + potionName +
                              "] , adding it to the config...");
  ...
  public static void loadTransactionMessages() {
    ...
    try (WarpedResultSet warpRS =
             plugin.getDatabaseHelper().selectAllMessages()) {
      ...
      while (rs.next()) {
        ...
        LinkedList<String> msgs = outGoingPlayerMessages.computeIfAbsent(
            ownerUUID, k -> new LinkedList<>());
        msgs.add(message);
      }
    } catch (SQLException e) {
      plugin.getLogger().log(
          Level.WARNING,
          "Could not load transaction messages from database. Skipping.", e);
    }
  }

  /**
   * @param player      The name of the player to message
   * @param message     The message to send them Sends the given player a
   *     message if they're online.
   *                    Else, if they're not online, queues it for them in the
   * database.
   * @param isUnlimited The shop is or unlimited
   *                    <p>
   *                    Deprecated for always use for bukkit deserialize method
   * (costing ~145ms)
   */
  @Deprecated
  public static void send(@NotNull UUID player, @NotNull String message,
                          boolean isUnlimited) {
    if (isUnlimited &&
        plugin.getConfig().getBoolean("shop.ignore-unlimited-shop-messages")) {
    ...
    if (!p.isOnline()) {
      LinkedList<String> msgs =
          outGoingPlayerMessages.getOrDefault(player, new LinkedList<>());
      msgs.add(message);
      outGoingPlayerMessages.put(player, msgs);
      plugin.getDatabaseHelper().sendMessage(player, message,
                                             System.currentTimeMillis());
    } else {
      if (p.getPlayer() != null) {
        if (msgData.length == 3) {
          try {
            plugin.getQuickChat().sendItemHologramChat(
                p.getPlayer(), msgData[0],
                Objects.requireNonNull(Util.deserialize(msgData[1])),
                msgData[2]);
          } catch (Exception any) {
   ...
   * @param player  The name of the player to message
   * @param message The message to send, if the given player are online it will
   *     be send immediately,
   *                Else, if they're not online, queues them in the database.
   */
  public static void send(@NotNull Shop shop, @NotNull UUID player,
    ...
    if (!p.isOnline()) {
      ...
      if (p.getPlayer() != null) {
        if (msgData.length == 3) {
          try {
            plugin.getQuickChat().sendItemHologramChat(
                p.getPlayer(), msgData[0], shop.getItem(), msgData[2]);
          } catch (Exception any) {
            Util.debugLog("Unknown error, send by plain text.");
            // Normal msg
            MsgUtil.sendMessage(p.getPlayer(), message);
  ...
  public static @NotNull String getSubString(@NotNull String text,
                                             @NotNull String left,
   ...
   */
  public static void sendControlPanelInfo(@NotNull CommandSender sender,
                                          @NotNull Shop shop) {
    if ((sender instanceof Player) &&
        !QuickShop.getPermissionManager().hasPermission(sender,
                                                        "quickshop.use") &&
        (shop.getOwner().equals(((Player)sender).getUniqueId()) ||
         !QuickShop.getPermissionManager().hasPermission(
             sender, "quickshop.other.control")) &&
        !InteractUtil.check(InteractUtil.Action.CONTROL,
                            ((Player)sender).isSneaking())) {

      return;
    }
    if (Util.fireCancellableEvent(
            new ShopControlPanelOpenEvent(shop, sender))) {
    ...
    chatSheetPrinter.printLine(
        MsgUtil.getMessage("controlpanel.infomation", sender));
    // Owner
    if (!QuickShop.getPermissionManager().hasPermission(sender,
                                                        "quickshop.setowner")) {
      chatSheetPrinter.printLine(
          MsgUtil.getMessage("menu.owner", sender, shop.ownerName()));
    } else {
      chatSheetPrinter.printSuggestedCmdLine(
          MsgUtil.getMessage(
              "controlpanel.setowner", sender,
              shop.ownerName() +
                  ((plugin.getConfig().getBoolean(
                        "shop.show-owner-uuid-in-controlpanel-if-op") &&
                    shop.isUnlimited())
                       ? (" (" + shop.getOwner() + ")")
                       : "")),
    ...
    if (QuickShop.getPermissionManager().hasPermission(sender,
                                                       "quickshop.unlimited")) {
      String text = MsgUtil.getMessage("controlpanel.unlimited", sender,
                                       bool2String(shop.isUnlimited()));
      String hoverText =
          MsgUtil.getMessage("controlpanel.unlimited-hover", sender);
      String clickCommand =
          MsgUtil.fillArgs("/qs silentunlimited {0}",
    ...
    if (QuickShop.getPermissionManager().hasPermission(
        ...
        QuickShop.getPermissionManager().hasPermission(
            sender, "quickshop.create.sell")) {
      if (shop.isSelling()) {
        String text = MsgUtil.getMessage("controlpanel.mode-selling", sender);
        String hoverText =
            MsgUtil.getMessage("controlpanel.mode-selling-hover", sender);
        String clickCommand = MsgUtil.fillArgs(
            "/qs silentbuy {0}", shop.getRuntimeRandomUniqueId().toString());
        chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
      } else if (shop.isBuying()) {
        String text = MsgUtil.getMessage("controlpanel.mode-buying", sender);
        String hoverText =
            MsgUtil.getMessage("controlpanel.mode-buying-hover", sender);
        String clickCommand = MsgUtil.fillArgs(
            "/qs silentsell {0}", shop.getRuntimeRandomUniqueId().toString());
        chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
      }
    }
    // Set Price
    if (QuickShop.getPermissionManager().hasPermission(
            sender, "quickshop.other.price") ||
        shop.getOwner().equals(((OfflinePlayer)sender).getUniqueId())) {
      String text =
          MsgUtil.fillArgs(MsgUtil.getMessage("controlpanel.price", sender),
    ...
    if (QuickShop.getInstance().isAllowStack()) {
      if (QuickShop.getPermissionManager().hasPermission(
              sender, "quickshop.other.amount") ||
          shop.getOwner().equals(((OfflinePlayer)sender).getUniqueId()) &&
              QuickShop.getPermissionManager().hasPermission(
                  sender, "quickshop.create.changeamount")) {
        String text =
            MsgUtil.getMessage("controlpanel.stack", sender,
                               Integer.toString(shop.getItem().getAmount()));
        String hoverText =
            MsgUtil.getMessage("controlpanel.stack-hover", sender);
    ...
    if (!shop.isUnlimited()) {
      ...
      if (QuickShop.getPermissionManager().hasPermission(sender,
                                                         "quickshop.refill")) {
        String text = MsgUtil.getMessage("controlpanel.refill", sender,
                                         String.valueOf(shop.getPrice()));
        String hoverText =
            MsgUtil.getMessage("controlpanel.refill-hover", sender);
      ...
      if (QuickShop.getPermissionManager().hasPermission(sender,
                                                         "quickshop.empty")) {
        String text = MsgUtil.getMessage("controlpanel.empty", sender,
                                         String.valueOf(shop.getPrice()));
        String hoverText =
            MsgUtil.getMessage("controlpanel.empty-hover", sender);
        String clickCommand = MsgUtil.fillArgs(
            "/qs silentempty {0}", shop.getRuntimeRandomUniqueId().toString());
        chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
      }
    }
    // Remove
    if (QuickShop.getPermissionManager().hasPermission(
            sender, "quickshop.other.destroy") ||
        shop.getOwner().equals(((OfflinePlayer)sender).getUniqueId())) {
      String text = MsgUtil.getMessage("controlpanel.remove", sender,
                                       String.valueOf(shop.getPrice()));
      String hoverText =
          MsgUtil.getMessage("controlpanel.remove-hover", sender);
      String clickCommand = MsgUtil.fillArgs(
          "/qs silentremove {0}", shop.getRuntimeRandomUniqueId().toString());
      chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
    }
  ...
  public static String getMessage(@NotNull UUID uuid, @NotNull String loc,
                                  @NotNull String... args) {
    ...
    try {
      ...
      if (player instanceof OfflinePlayer) {
        if (plugin.getPlaceHolderAPI() != null &&
            plugin.getPlaceHolderAPI().isEnabled() &&
            plugin.getConfig().getBoolean("plugin.PlaceHolderAPI")) {
          try {
            filled =
                PlaceholderAPI.setPlaceholders((OfflinePlayer)player, filled);
          } catch (Exception ignored) {
            if (((OfflinePlayer)player).getPlayer() != null) {
              try {
                filled = PlaceholderAPI.setPlaceholders(
                    ((OfflinePlayer)player).getPlayer(), filled);
  ...
  /**
   * Translate boolean value to String, the symbon is changeable by language
   * file.
   *
  ...
  public static void sendGlobalAlert(@Nullable String content) {
    if (content == null) {
      ...
      Throwable throwable = new Throwable(
          "Known issue: Global Alert accepted null string, what the fuck");
   ...
   */
  public static void sendPurchaseSuccess(@NotNull UUID purchaser,
    ...
    chatSheetPrinter.printHeader();
    chatSheetPrinter.printLine(
        MsgUtil.getMessage("menu.successful-purchase", sender));
    chatSheetPrinter.printLine(MsgUtil.getMessage(
        "menu.item-name-and-price", sender,
   ...
   * @param amount Trading item amounts.
   */
  public static void sendSellSuccess(@NotNull UUID seller, @NotNull Shop shop,
                                     int amount) {
    ...
    chatSheetPrinter.printHeader();
    chatSheetPrinter.printLine(
        MsgUtil.getMessage("menu.successfully-sold", sender));
    chatSheetPrinter.printLine(MsgUtil.getMessage(
        "menu.item-name-and-price", sender, Integer.toString(amount),
        Util.getItemStackName(shop.getItem()),
        Util.format(amount * shop.getPrice(), shop)));
    if (plugin.getConfig().getBoolean("show-tax")) {
      ...
      if (tax != 0) {
        if (!seller.equals(shop.getOwner())) {
          chatSheetPrinter.printLine(MsgUtil.getMessage(
              "menu.sell-tax", sender, Util.format(tax * total, shop)));
        } else {
          chatSheetPrinter.printLine(
              MsgUtil.getMessage("menu.sell-tax-self", sender));
        }
      }
    }
    printEnchantment(sender, shop, chatSheetPrinter);
  ...
  private static void printEnchantment(@NotNull Player p, @NotNull Shop shop,
                                       ChatSheetPrinter chatSheetPrinter) {
    if (shop.getItem().hasItemMeta() &&
        shop.getItem().getItemMeta().hasItemFlag(ItemFlag.HIDE_ENCHANTS) &&
        plugin.getConfig().getBoolean("respect-item-flag")) {
      return;
    }
    Map<Enchantment, Integer> enchs = new HashMap<>();
    if (shop.getItem().hasItemMeta() &&
        shop.getItem().getItemMeta().hasEnchants()) {
    ...
    if (shop.getItem().getItemMeta() instanceof EnchantmentStorageMeta) {
      EnchantmentStorageMeta stor =
          (EnchantmentStorageMeta)shop.getItem().getItemMeta();
      stor.getStoredEnchants();
      enchs = stor.getStoredEnchants();
      if (!enchs.isEmpty()) {
        chatSheetPrinter.printCenterLine(
            MsgUtil.getMessage("menu.stored-enchants", p));
        printEnchantment(chatSheetPrinter, enchs);
      }
    }
  }

  private static void printEnchantment(ChatSheetPrinter chatSheetPrinter,
                                       Map<Enchantment, Integer> enchs) {
    for (Entry<Enchantment, Integer> entries : enchs.entrySet()) {
      chatSheetPrinter.printLine(ChatColor.YELLOW +
                                 MsgUtil.getEnchi18n(entries.getKey()) + " " +
                                 RomanNumber.toRoman(entries.getValue()));
    }
  }
  ...
  public static void sendShopInfo(@NotNull Player p, @NotNull Shop shop) {
    ...
    chatSheetPrinter.printLine(
        MsgUtil.getMessage("menu.owner", p, shop.ownerName()));
    // Enabled
    plugin.getQuickChat().send(
        p, plugin.getQuickChat().getItemHologramChat(
               shop, items, p,
               ChatColor.DARK_PURPLE +
                   MsgUtil.getMessage("tableformat.left_begin", p) +
                   MsgUtil.getMessage("menu.item", p,
                                      Util.getItemStackName(items)) +
                   "  "));
    if (Util.isTool(items.getType())) {
      chatSheetPrinter.printLine(MsgUtil.getMessage(
          "menu.damage-percent-remaining", p, Util.getToolPercentage(items)));
    }
    if (shop.isSelling()) {
      if (shop.getRemainingStock() == -1) {
        chatSheetPrinter.printLine(MsgUtil.getMessage(
            "menu.stock", p, MsgUtil.getMessage("signs.unlimited", p)));
      } else {
        chatSheetPrinter.printLine(MsgUtil.getMessage(
            "menu.stock", p, Integer.toString(shop.getRemainingStock())));
      }
    } else {
      if (shop.getRemainingSpace() == -1) {
        chatSheetPrinter.printLine(MsgUtil.getMessage(
            "menu.space", p, MsgUtil.getMessage("signs.unlimited", p)));
      } else {
        chatSheetPrinter.printLine(MsgUtil.getMessage(
            "menu.space", p, Integer.toString(shop.getRemainingSpace())));
      }
    }
    if (shop.getItem().getAmount() == 1) {
      chatSheetPrinter.printLine(MsgUtil.getMessage(
          "menu.price-per", p, Util.getItemStackName(shop.getItem()),
          Util.format(shop.getPrice(), shop)));
    } else {
      chatSheetPrinter.printLine(MsgUtil.getMessage(
          "menu.price-per-stack", p, Util.getItemStackName(shop.getItem()),
          Util.format(shop.getPrice(), shop),
          Integer.toString(shop.getItem().getAmount())));
    }
    if (shop.isBuying()) {
      chatSheetPrinter.printLine(
          MsgUtil.getMessage("menu.this-shop-is-buying", p));
    } else {
      chatSheetPrinter.printLine(
          MsgUtil.getMessage("menu.this-shop-is-selling", p));
    }
    ...
    if (items.getItemMeta() instanceof PotionMeta) {
      ...
      PotionEffectType potionEffectType =
          potionMeta.getBasePotionData().getType().getEffectType();
      if (potionEffectType != null) {
        chatSheetPrinter.printLine(MsgUtil.getMessage("menu.effects", p));
        chatSheetPrinter.printLine(ChatColor.YELLOW +
                                   MsgUtil.getPotioni18n(potionEffectType));
      }
  ...
  public static IFile getI18nFile() { return messagei18n; }
  ...
  public static void debugStackTrace(StackTraceElement[] traces) {
    ...
    for (StackTraceElement stackTraceElement : traces) {
      ...
      Util.debugLog("[TRACE]  [" + className + "] [" + methodName + "] (" +
                    fileName + ":" + codeLine + ") ");
    }
  }
  ...
  private static void updateMessages(int selectedVersion) {
    ...
    if (!messagei18n.getString("language-name").get().equals(languageName)) {
      Util.debugLog("Language name " +
                    messagei18n.getString("language-name").get() +
                    " not matched with " + languageName);
      File pending = new File(plugin.getDataFolder(), "messages.json");
      try {
        Files.move(
            pending.toPath(),
            new File(plugin.getDataFolder(),
                     "messages-bak-" + UUID.randomUUID().toString() + ".json")
                .toPath());
    ...
    builtInLang =
        new JSONFile(plugin, "lang/" + languageName + "/messages.json");
    ...
    if (selectedVersion == 1) {
      ...
      setAndUpdate("controlpanel.refill",
                   "&aRefill: Refill the shop items &e[&d&lOK&e]");
      setAndUpdate("controlpanel.refill-hover",
                   "&eLooking you want changing shop and click to refill.");
      setAndUpdate("controlpanel.empty",
                   "&aEmpty: Remove shop all items &e[&d&lOK&e]");
      setAndUpdate("controlpanel.empty-hover",
                   "&eLooking you want changing shop and click to clear.");
    ...
    if (selectedVersion == 6) {
      ...
      setAndUpdate("break-shop-use-supertool",
                   "&eYou break the shop by use SuperTool.");
      messagei18n.set(
          "no-creative-break",
    ...
    if (selectedVersion == 7) {
      setAndUpdate(
          "failed-to-put-sign",
          "&cNo enough space around the shop to place infomation sign.");
    ...
    if (selectedVersion == 8) {
      ...
      setAndUpdate("command.description.paste",
                   "&eAuto upload server data to Pastebin");
    ...
    if (selectedVersion == 10) {
      setAndUpdate(
          "price-too-high",
          "&cShop price too high! You can't create price higher than {0} shop.");
    ...
    if (selectedVersion == 11) {
      setAndUpdate(
          "unknown-player",
          "&cTarget player not exist, please check username your typed.");
      setAndUpdate("shop-staff-cleared",
                   "&aSuccessfully remove all staff for your shop.");
      setAndUpdate("shop-staff-added",
                   "&aSuccessfully add {0} to your shop staffs.");
      setAndUpdate("shop-staff-deleted",
                   "&aSuccessfully remove {0} to your shop staffs.");
      setAndUpdate("command.wrong-args",
                   "&cParameters not matched, use /qs help to check help");
    ...
    if (selectedVersion == 13) {
      ...
      setAndUpdate("success-change-owner-to-server",
                   "&aSuccessfully set shop owner to Server.");
    ...
    if (selectedVersion == 27) {
      ...
      setAndUpdate(
          "backup-failed",
          "Cannot backup the database, check the console for details.");
      setAndUpdate(
          "translate-not-completed-yet-click",
          "The translation of language {0} has completed {1}, Do you want help us to improve the translation? Click Here!");
      setAndUpdate(
          "translate-not-completed-yet-url",
          "The translation of language {0} has completed {1}, Do you want help us to improve the translation? Browse: {2}");
      ...
      setAndUpdate("not-managed-shop",
                   "You isn't the owner or moderator of the shop");
    ...
    if (selectedVersion == 30) {
      setAndUpdate(
          "no-creative-break",
          "&cYou cannot break other players shops in the creative mode, switch to survival mode or try to use supertool {0} instead.");
      setAndUpdate("trading-in-creative-mode-is-disabled",
                   "&cYou cannot trade with shop in the creative mode.");
      setAndUpdate("supertool-is-disabled",
                   "&cSupertool is disabled, cannot break any shop.");
    ...
    if (selectedVersion == 31) {
      ...
      setAndUpdate("command.description.language",
                   "&eSwitch the language that quickshop using");
      ...
      setAndUpdate(
          "controlpanel.stack-hover",
          "&eClick to set the item amounts each stack. Set to 1 for normal behavior.");
      setAndUpdate(
          "shop-now-freezed",
          "&aYou are frozen the shop, and nobody can trade with this shop now!");
      setAndUpdate("shop-nolonger-freezed",
                   "&aYou are unfrozen the shop, it back to normal now!");
      setAndUpdate("shop-freezed-at-location",
                   "&eYour shop {0} at {1} got frozen!");
      setAndUpdate(
          "shop-cannot-trade-when-freezing",
          "&cYou cannot trade with this shop cause it under freezing.");
      setAndUpdate("denied-put-in-item",
                   "&cThis item cannot put in this shop!");
      setAndUpdate(
          "how-many-buy-stack",
          "&aEnter how many bulks you wish to &bBUY&a in chat. There have &e{0}&a items in each bulk and You can buy &e{1}&a bulks. Enter &ball&a to buy them all.");
      setAndUpdate(
          "how-many-sell-stack",
          "&aEnter how many bulks you wish to &dSELL&a in chat. There have &e{0}&a items in each bulk and You have &e{1}&a bulks available. Enter &ball&a to sell them all.");
    ...
    if (selectedVersion == 33) {
      setAndUpdate("integrations-check-failed-create",
                   "Integrations denied the shop creation");
      setAndUpdate("integrations-check-failed-trade",
                   "Integrations denied the shop trade");
    ...
    if (selectedVersion == 35) {
      ...
      setAndUpdate("controlpanel.stack",
                   "&aPer bulk amount: &b{0} &e[&d&lChange&e]");
      setAndUpdate(
          "controlpanel.stack-hover",
          "&eClick to set the amount of item per bulk. Set to 1 for normal behaviour.");
      ...
      setAndUpdate(
          ...
          "&aEnter in chat, how much you wish to trade {1}x &e{0}&a for.");
      setAndUpdate("command.bulk-size-not-set", "&cUsage: /qs size <amount>");
      setAndUpdate("command.bulk-size-now", "&aNow trading {0}x {1}");
      setAndUpdate(
          "command.invalid-bulk-amount",
          "&cThe given value {0} larger than max stack size or lower than one");
      setAndUpdate("command.description.size",
                   "&eChange per bulk amount of a shop");
      setAndUpdate("command.no-trade-item",
                   "&aPlease hold a trade item to change in main hand");
    ...
    if (selectedVersion == 36) {
      ...
      setAndUpdate(
          "how-much-to-trade-for",
          "&aEnter in chat, how much you wish to trade &e{1}x {0}&a for.");
    ...
    if (selectedVersion == 37) {
      ...
      setAndUpdate("command.description.removeall",
                   "&eRemove ALL shops of a specified player");
    ...
    if (selectedVersion == 38) {
      setAndUpdate("integrations-check-failed-create",
                   "&cIntegration {0} denied the shop-creation");
      setAndUpdate("integrations-check-failed-trade",
                   "&cIntegration {0} denied the Shop trading");
      setAndUpdate(
          "3rd-plugin-build-check-failed",
          "&c3rd party plugin &l{0}&r&c denied the permission checks, did you have permission setup in there?");
      setAndUpdate("language-version", 39);
      selectedVersion = 39;
    }
    if (selectedVersion == 39) {
      setAndUpdate("command.transfer-success",
                   "&aTransferred &e{0} &ashop(s) to &e{1}");
      setAndUpdate("command.transfer-success-other",
                   "&aTransferred &e{0} {1}&a's shop(s) to &e{2}");
      setAndUpdate("command.description.transfer",
                   "&eTransfer someone's ALL shops to other");
    ...
    if (selectedVersion == 41) {
      setAndUpdate("shops-removed-in-world",
                   "&eTotal &b{0}&e shops has been deleted in world &b{1}&e.");
      setAndUpdate("command.description.removeworld",
                   "&eRemove ALL shops in a specified world");
    ...
    if (selectedVersion == 42) {
      setAndUpdate(
          ...
          "&c{0} purchased {1} {2} from your shop, and you earned {3} ({4} in taxes).");
      setAndUpdate(
          "player-bought-from-your-store",
          "&c{0} purchased {1} {2} from your shop, and you earned {3}.");
    ...
    if (selectedVersion == 43) {
      ...
      setAndUpdate(
          "nearby-shop-entry",
          "&a- Info:{0} &aPrice:&b{1} &ax:&b{2} &ay:&b{3} &az:&b{4} &adistance: &b{5} &ablock(s)");
      setAndUpdate("language-version", ++selectedVersion);
    }
    if (selectedVersion == 44) {
      setAndUpdate("nearby-shop-this-way",
                   "&aShop is {0} blocks away from you.");
      setAndUpdate("language-version", ++selectedVersion);
    }
    if (selectedVersion == 45) {
      setAndUpdate("exceeded-maximum",
                   "&cThe value has exceeded the maximum java value.");
      setAndUpdate("language-version", ++selectedVersion);
    }
    if (selectedVersion == 46) {
      setAndUpdate(
          "currency-not-exists",
          "&cCannot found currency that you want set to, maybe is bad spelling or that currency not available in this world.");
      setAndUpdate("currency-set", "&aSuccessfully set shop currency to {0}.");
      setAndUpdate(
          "currency-unset",
          "&aSuccessfully remove shop currency setup, will use default now.");
      setAndUpdate("command.description.currency",
                   "&eSet or remove the currency setting on shop");
      setAndUpdate("controlpanel.currency", "&aCurrency: &b{0} &e[&d&lSet&e]");
      setAndUpdate("controlpanel.currency-hover",
                   "&eClick to set or remove the currency that shop using");
      setAndUpdate(
          "currency-not-support",
          "&cThe economy plugin doesn't support multi-currency feature.");
      setAndUpdate("language-version", ++selectedVersion);
    }
    if (selectedVersion == 47) {
      setAndUpdate(
          "forbidden-vanilla-behavior",
          "&cThe operation is forbidden due to is not consist with vanilla behavior");
      setAndUpdate("language-version", ++selectedVersion);
    }
    if (selectedVersion == 48) {
      ...
      setAndUpdate(
          "item-not-exist",
          "&cThe item &e{0} &cnot exists, please check your spelling.");
      setAndUpdate("language-version", ++selectedVersion);
    }
    setAndUpdate("_comment",
                 "Please edit this file after format with json formatter");
    messagei18n.save();
    messagei18n.loadFromString(Util.parseColours(messagei18n.saveToString()));
  }
  ...
  public static void sendColoredMessage(@NotNull CommandSender sender,
  ...
  public static void sendMessage(@NotNull UUID sender,
                                 @Nullable String... messages) {
    sendMessage(Bukkit.getPlayer(sender), messages);
  }

  public static void sendMessage(@Nullable CommandSender sender,
                                 @Nullable String... messages) {
    if (messages == null) {
