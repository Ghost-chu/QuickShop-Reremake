 ...
 * ShopProtectionListener.java Copyright (C) PotatoCraft Studio and contributors
 *
...
public class ShopProtectionListener extends ProtectionListenerBase {
  ...
  public ShopProtectionListener(@NotNull QuickShop plugin,
    ...
    super(plugin, cache);
    this.sendProtectionAlert =
        plugin.getConfig().getBoolean("send-shop-protection-alert", false);
    useEnhanceProtection =
        plugin.getConfig().getBoolean("shop.enchance-shop-protect");
  ...
  public void scanAndFixPaperListener() {
    ...
    Util.debugLog(
        "QuickShop is scanning all worlds settings about disableHopperMoveEvents disabled worlds");
    Bukkit.getWorlds().forEach(world -> {
      ...
      try {
        ...
        Object paperWorldConfig = worldServer.getClass()
                                      .getSuperclass()
                                      .getDeclaredField("paperConfig")
                                      .get(worldServer);
        boolean disableHopperMoveEvents =
            paperWorldConfig.getClass()
                .getDeclaredField("disableHopperMoveEvents")
                .getBoolean(paperWorldConfig);
        if (disableHopperMoveEvents) {
          ...
          plugin.getLogger().warning(
              "If you still want keep enable disableHopperMoveEvents enables "
              +
              "in this world, please disable Hopper protection or make sure no shops in this world.");
          paperWorldConfig.getClass()
              .getDeclaredField("disableHopperMoveEvents")
              .setBoolean(paperWorldConfig, false);
          File serverRoot =
          ...
          if (paperConfigYaml.exists()) {
            YamlConfiguration yamlConfiguration =
                YamlConfiguration.loadConfiguration(paperConfigYaml);
            ConfigurationSection worldsSection = Objects.requireNonNull(
                yamlConfiguration.getConfigurationSection("world-settings"));
            ConfigurationSection worldSection;
            if (Objects.requireNonNull(worldsSection)
                    .getConfigurationSection(world.getName()) == null) {
              worldSection = worldsSection.getConfigurationSection("default");
            } else {
              worldSection =
                  worldsSection.getConfigurationSection(world.getName());
            }
            Objects.requireNonNull(worldSection)
                .set("hopper.disable-move-event", false);
            Objects.requireNonNull(worldSection)
                .set(
                    "hopper.disable-move-event-quickshop-tips",
        ...
        plugin.getLogger().log(
            Level.WARNING,
            "Failed to automatic disable disable-move-event for world [" +
                world.getName() +
                "], please disable it by yourself or player can steal items from shops.",
            e);
      }
    });
  }
    ...
    for (int i = 0, a = e.blockList().size(); i < a; i++) {
      ...
      if (shop != null) {
        if (plugin.getConfig().getBoolean("protect.explode")) {
          ...
          plugin.log("Deleting shop " + shop +
                     " request by block break (explode).");
          shop.delete();
        }
      }
    }
  ...
  public void onBlockRedstoneChange(BlockRedstoneEvent event) {
    ...
    // " + shop); Util.debugLog(ChatColor.RED + "[QuickShop][Exploit alert]
    // Redstone was activated on the following shop " + shop);
  }
  ...
  public void onInventoryMove(InventoryMoveItemEvent event) {
    ...
    if (sendProtectionAlert) {
      MsgUtil.sendGlobalAlert("[DisplayGuard] Defend a item steal action at" +
                              location);
    }
  }
  ...
  public void onStructureGrow(StructureGrowEvent event) {
    ...
    for (BlockState blockstate : event.getBlocks()) {
      ...
      event.setCancelled(true);
      return;
      // plugin.getLogger().warning("[Exploit Alert] a StructureGrowing tried to
      // break the shop of "
      // + shop);
      // Util.sendMessageToOps(ChatColor.RED + "[QuickShop][Exploit alert] A
      // StructureGrowing tried to break the shop of " + shop);
    }
  }
