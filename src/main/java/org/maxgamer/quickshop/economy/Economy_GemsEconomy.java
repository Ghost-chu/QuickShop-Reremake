 ...
 * Economy_GemsEconomy.java Copyright (C) PotatoCraft Studio and contributors
 *
...
public class Economy_GemsEconomy implements EconomyCore {
  ...
  @Getter @Setter private GemsEconomyAPI api;
  ...
  private void setupEconomy() { this.api = new GemsEconomyAPI(); }

  @Nullable
  private Currency getCurrency(@NotNull World world,
  ...
  private String formatInternal(double balance, @NotNull World world,
                                @Nullable String currency) {
    if (!isValid()) {
      return "Error";
    }
